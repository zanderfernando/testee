...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '𝘛𝘰𝘮𝘢 𝘶𝘮 𝘤𝘩𝘢𝘻𝘪𝘯 𝘢𝘪́ 𝘲𝘶𝘦 𝘦𝘶 𝘫𝘢́ 𝘦𝘴𝘵𝘰𝘶 𝘧𝘢𝘻𝘦𝘯𝘥𝘰...🕘',
				success: 'SUCESSO MLK',
				error: {
					stick: 'Deu erro, tente novamente :/',
					Iv: 'Link inválido.'
				},
				only: {
					group: '❌*𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝙎𝙊 𝙋𝙊𝘿𝙀 𝙎𝙀 𝙐𝙎𝘼𝘿𝙊 𝙀𝙈 𝙂𝙍𝙐𝙋𝙊 ❌',
					ownerG: '*「SÓ O DONO USA ESSE COMANDO」*',
					ownerB: '*「SÓ O DONO USA ESSE COMANDO」*',
					admin: '❌𝘚𝘢𝘪 𝘥𝘢𝘦 𝘮𝘭𝘬, 𝘷𝘤 𝘦́ 𝘢𝘱𝘦𝘯𝘢𝘴 𝘶𝘮 𝘮𝘦𝘮𝘣𝘳𝘰',
					Badmin: '[❗] ESTE COMANDO SO PODE SER USADO QUANDO BOT VIRA ADM'
				}
			}
			...
			switch(command) {
				case 'help':
				case 'menu':
client.sendMessage(from, help(prefix) , text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "𝐙𝐀𝐍𝐃𝐄𝐑 𝐁𝐎𝐓☔", 'jpegThumbnail': fs.readFileSync('kk/sticker/botlogo.webp')}}}})					
break
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'CADE O NOME DO USUÁRIO', text, {quoted: mek})
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('Quem é você?kkk') 
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '𝗗𝗲 𝗺𝗲𝗺𝗯𝗿𝗼 𝘃𝗼𝗰𝗲̂ 𝗲́ 𝗺𝗲𝗹𝗵𝗼𝗿 𝗸𝗸𝗸𝗸\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Não sou adivinha caralho, quem você quer adicionar?') 
					if (args[0].startsWith('08')) return reply('Escreve o número direito burro')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Não foi possível adicionar esse macaco' )
					}
					break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Olha o ban kkkk : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
